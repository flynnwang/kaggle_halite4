...
# https://www.kaggle.com/david1013/three-sigma/output
  ...
  for player in range(len(obs.players)):
    ...
    for shipyard in shipyards:
      ...
      if player == obs.player:
        ...
        for spot in [(x, y), (c(x - 1), y), (c(x + 1), y), (x, c(y - 1)),
                     (x, c(y + 1))]:
...
def threat(x, y, player, ship_halite):
  ...
  return ((spot["ship"] != None and spot["ship"] != player and
           spot["enemy_halite"] <= ship_halite) or
          (spot["shipyard"] != None and spot["shipyard"] != player))
...
def clear(x, y, player, halite_onboard, moveable=False):
  ...
  if ((game_map[x][y]["shipyard"] == player or
       game_map[x][y]["shipyard"] == None) and
      ((game_map[x][y]["ship"] == player and moveable) or
       (game_map[x][y]["ship"] == None) or
       (game_map[x][y]["ship"] != player and
        game_map[x][y]["enemy_halite"] > halite_onboard))):
    return True
  return False


def clearArea(x, y, player, halite_onboard, moveable=False):
  return (clear(x, y, player, halite_onboard, moveable) and
          not enemy_ship_near(x, y, player, halite_onboard))
...
EXPECTED_MINE_FRAC = np.sum(
    [0.25 * (1 - 0.25 + 0.02)**i for i in range(int(EXPECTED_MINE_TURNS))])
...
def findBestSpot(x_pos,
  ...
  if surrounded(x_pos, y_pos, player, halite_onboard):
    if halite_onboard > conf.convertCost or my_halite > conf.convertCost:
      logit('emergency conversion at ({}, {}) to preserve {} halite'.format(
          x_pos, y_pos, halite_onboard))
      return (x_pos, y_pos, np.floor(halite_onboard) + conf.convertCost,
              'conversion')
    else:
      logit(
          'surrounded at ({}, {}) but not enough cash for emergency conversion'.
          format(x_pos, y_pos))

  ooo = onlyOneOption(x_pos, y_pos, player, halite_onboard, moveable)
  if ooo is not None:
    logit('only one option from ({}, {}), heading {}, {}'.format(
        x_pos, y_pos, ooo[0], ooo[1]))
    return (c(x_pos + ooo[0]), c(y_pos + ooo[1]),
            np.floor(halite_onboard) + conf.convertCost, 'only one option')
  ...
  return_penalty = ((ship_value - conf.spawnCost) if
                    (my_halite > conf.spawnCost and
                     ship_value > conf.spawnCost) else 0)
  ...
  for shipyard_coords in my_shipyards_coords:
    ...
    if (not clearArea(x, y, player, halite_onboard, moveable) or (not clearArea(
        (x_pos + x_dir) % conf.size, y_pos, player, halite_onboard,
        moveable) and not clearArea(x_pos, (y_pos + y_dir) % conf.size, player,
                                    halite_onboard, moveable))):
      continue

    dist = distanceTo(x, y, x_pos,
                      y_pos)  #  * (step + base_step) ** 2 / (base_step + 400)

    if dist < nearest_base:
      nearest_base = dist

    spot_value = (halite_onboard * RETURN_CHANCE /
                  (dist * MOVE_MULTIPLIER + 1e-6) * np.min(
                      (np.sqrt(step / 30), 1)) * np.clip(
                          (ship_value / conf.spawnCost), 1, 1.5))
    if spot_value > return_value:
      return_value = spot_value
    if spot_value > best_spot[2] and not enemy_ship_near(
        x, y, player, halite_onboard):
  ...
  for x in range(conf.size):
    for y in range(conf.size):
      ...
      if (not clearArea(x, y, player, halite_onboard, moveable) or
          (not clearArea(
              (x_pos + x_dir) % conf.size, y_pos, player, halite_onboard,
              moveable) and not clearArea(x_pos, (y_pos + y_dir) % conf.size,
                                          player, halite_onboard, moveable))):
        continue
      ...
      if game_map[x][y]["enemy_halite"] > halite_onboard:
        ...
        spot_raid_value = (
            (game_map[x][y]["enemy_halite"] * RAID_MULTIPLIER -
             ...
             (RLEN_PENALTY if
              game_map[x][y]['lightest_enemy_nearby'] < halite_onboard else 0) +
             (RLE3_PENALTY if game_map[x][y]['lightest_enemy_within_3'] <
              halite_onboard else 0)) /
      ...
      if ((must_move or
           game_map[x][y]["halite"] > current_halite * ship_inertia) and
          game_map[x][y]["targeted"] == 0):
        dist = distanceTo(x, y, x_pos, y_pos)
        spot_value = (
            game_map[x][y]["halite"] * EXPECTED_MINE_FRAC /
            (EXPECTED_MINE_TURNS + dist * MOVE_MULTIPLIER + nearest_base *
             MINE_BASE_DIST_MULT) * np.exp(-DISTANCE_DECAY * dist) *
            (0.7 if game_map[x][y]["touching_base"] else 1) +
            ((return_value * RETURN_RESIDUAL -
              return_penalty) if game_map[x][y]["touching_base"] else 0) +
            ...
            (MLET_PENALTY if
             (game_map[x][y]['lightest_enemy_touching'] < halite_onboard +
              game_map[x][y]["halite"] * MLET_FWD) else 0) +
            (MLEN_PENALTY if
             (game_map[x][y]['lightest_enemy_nearby'] < halite_onboard +
              game_map[x][y]["halite"] * MLEN_FWD) else 0) +
            (MLE3_PENALTY if
             (game_map[x][y]['lightest_enemy_within_3'] < halite_onboard +
              game_map[x][y]["halite"] * MLE3_FWD) else 0) +
  ...
  if best_spot[3] == 'raiding':
    logit('attempting raid of ({}, {}) from ({}, {}) to gain {} halite'.format(
        best_spot[0], best_spot[1], x_pos, y_pos,
        game_map[best_spot[0]][best_spot[1]]["enemy_halite"]))
...
def moveTo(x_initial, y_initial, x_target, y_target, ship_id, halite_onboard,
           player, actions):
  ...
  if x_dist > y_dist:
    # move X first if can;
    if clearArea((x_initial + x_dir) % conf.size, y_initial, player,
                 halite_onboard):
      action = ('WEST' if x_dir < 0 else 'EAST')
    elif clearArea(x_initial, (y_initial + y_dir) % conf.size, player,
                   halite_onboard):
      action = ('NORTH' if y_dir < 0 else 'SOUTH')

  else:
    # move Y first if can
    if clearArea(x_initial, (y_initial + y_dir) % conf.size, player,
                 halite_onboard):
      action = ('NORTH' if y_dir < 0 else 'SOUTH')
    elif clearArea((x_initial + x_dir) % conf.size, y_initial, player,
                   halite_onboard):
      action = ('WEST' if x_dir < 0 else 'EAST')
...
def orderBasesForSpawning(shipyards_keys, my_shipyards_coords, player,
                          my_ships_coords):
    ...
    for xs in range(-RADIUS, RADIUS + 1):
      for ys in range(-RADIUS, RADIUS + 1):
        ...
        nearby_enemy_halite += (game_map[c(x + xs)][c(y + ys)]["enemy_halite"] *
                                np.exp(-ENEMY_DISTANCE_DECAY *
                                       (abs(xs) + abs(ys))))

    for s in my_ships_coords:
      nearby_ships += (1 / (BASE_SHIP_TURNS + distanceTo(s[0], s[1], x, y)))

    base_values[yard_idx] = (
        (nearby_halite + RAIDING_EFFICIENCY * nearby_enemy_halite) /
...
def getShipValue(num_ships, step):
  return ((SHIP_MINE_PCT * getAverageHalite()**AVG_HALITE_PWR +
          ...
          (10 / num_ships) * (400 - step) + (1000 if num_ships < 3 else 0) +
          (3000 if num_ships < 2 else 0))


def strategicShipSpawning(my_halite, actions, num_ships, shipyards_keys,
                          my_shipyards_coords, step, player):
...
def timeout(start_time, player, step, rd, ship, config):
  ...
  if (datetime.datetime.now() - start_time).microseconds > TIMEOUT:
    print('AGENT {} TIMED OUT ON STEP {} - for Round {} and ship number {}'.
          format(player, step, rd, ship))
    return True
  else:
    return False
...
def swarm_agent(obs, config):
  ...
  reset_game_map(obs)
  my_shipyards_coords, my_ships_coords = get_my_units_coords_and_update_game_map(
      obs)
  ...
  for i in range(len(my_ships_coords)):
    ...
    x_target, y_target, spot_value, purpose = findBestSpot(
        x, y, obs.player, my_halite, halite_onboard, my_shipyards_coords,
        num_shipyards, avg_halite, obs.step, len(ships_keys), False)
    ...
    if purpose == 'only one option':
      logit(
          'only one option for ship at ({}, {}) with {} halite onboard'.format(
              x, y, halite_onboard))
  ...
  for i in range(len(my_ships_coords)):
    ...
    if game_map[x][y]["please_move"] > move_values[i]:
      ...
      logit("please move off ({}, {}) with weight {}".format(
          x, y, move_values[i]))
  ...
  for i in range(len(my_ships_coords)):
    ...
    if ((num_shipyards == 0 and (my_halite >= conf.convertCost or
                                 halite_onboard >= convert_plus_spawn_cost)) or
        (halite_onboard >= convert_plus_spawn_cost * SHIP_TO_BASE_MULT)):
    ...
    # '''final dropoff or must dropoff'''
          ...
          ((halite_onboard > MIN_FINAL_DROPOFF) or
           (halite_onboard > np.percentile([s[1] for s in ships_values],
                                           PCTILE_DROPOFF))) or
          (halite_onboard > MUST_DROPOFF)):

      if len(my_shipyards_coords) > 0:
        closest_yard, min_dist = findNearestYard(my_shipyards_coords, x, y)
        actions = moveTo(x, y, *my_shipyards_coords[closest_yard],
                         ships_keys[i], halite_onboard, obs.player, actions)

    # '''figure out best move'''
    else:
      x_target, y_target, new_spot_halite, purpose = findBestSpot(
          x, y, obs.player, my_halite, halite_onboard, my_shipyards_coords,
          num_shipyards, avg_halite, obs.step, len(ships_keys), True)
      if not (x_target == x and y_target == y):
        #                 logit('aiming to get to ({}, {}) from ({}, {})'.format(x_target,y_target, x ,y))
        actions = moveTo(x, y, x_target, y_target, ships_keys[i],
                         halite_onboard, obs.player, actions)
  ...
  if my_halite >= conf.spawnCost:
    shipyards_keys, my_shipyards_coords = orderBasesForSpawning(
        shipyards_keys, my_shipyards_coords, obs.player, my_ships_coords)
  ...
  # strategic spawning:
    ...
    my_halite, actions = strategicShipSpawning(my_halite, actions,
                                               len(ships_keys), shipyards_keys,
                                               my_shipyards_coords, obs.step,
                                               obs.player)
